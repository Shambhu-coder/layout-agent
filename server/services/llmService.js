/**
 * LLM Service — OpenRouter (deepseek/deepseek-chat)
...
export async function callLLM(systemPrompt, history, userMessage) {
  ...
  if (!parsed.explanation || !parsed.updatedLayout) {
    throw new Error(
      "LLM response missing required fields (explanation or updatedLayout)",
    );
  }
...
export function activeProvider() {
  return process.env.OPENROUTER_API_KEY ? "openrouter/deepseek-chat" : "none";
}
